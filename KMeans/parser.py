...
# Behavior : Either a raw file path or a config path (yaml config) 

# If the config path is provided,
# $ aa-kmap --config_path <path_to_yaml_file> --cluster_count 10 --frequency_list 1000 2000 --save_path <path_to_save_file> 

# where the --options are optional and act as overrides for the existing configuration in the YAML file but it will also save a new file with it creating like a chain of files with further specified properties which will be encoded in the filename.

# it loads the YAML file, applies overrides if any are provided, with the --option flag and saves the updated configuration to a new file.

# If a raw path is provided, it creates a new configuration file with default values and saves it to the specified save path, also with a timestamp in the filename with options allowed IN THIS CASE OPTIONS ARE MORE EXPECTED BUT THE BEHavior doesnt really change.

# Naming convention: <raw_path_name>_